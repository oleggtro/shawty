...
func CreateSession(state util.State, subject pgtype.UUID) (*Session, error) {
	...
	row := state.Db.QueryRow(context.Background(), "WITH ins AS (INSERT INTO sessions (subject) VALUES ($1) ON CONFLICT (subject) DO NOTHING RETURNING * ) SELECT * FROM ins UNION SELECT * FROM sessions WHERE subject = $1;", subject)
